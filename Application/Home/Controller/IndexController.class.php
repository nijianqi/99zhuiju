...
class IndexController extends BaseController
{
    public function index()
    {
        ...
        $open['open'] = 1;
        $count = $article->where("article.tid != 14")->count();
        $Page = new \Think\Page($count, 15);
        $show = $Page->show();
        $tptg = $article->where($open)->where("article.tid != 14")->order('id DESC')->limit($Page->firstRow . ',' . $Page->listRows)->select();
    ...
    public function category()
    {
        ...
        if ($c) {
            ...
            $Page = new \Think\Page($count, 15);
            $show = $Page->show();
            $tptj = $article->where("tid={$id}")->where($open)->order('time DESC')->limit($Page->firstRow . ',' . $Page->listRows)->select();
